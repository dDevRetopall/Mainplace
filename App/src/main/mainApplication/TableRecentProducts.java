...
import utils.threadUtils.TimerScheduler;

public class TableRecentProducts extends JScrollPane {
	...
	JPanel p = new JPanel() {
		...
		public Dimension getPreferredSize() {
			return new Dimension(400, getYLenght());
		}
	};

	TimerScheduler ts;

	public TableRecentProducts() {
		...
		getViewport().setPreferredSize(new Dimension(400, 300));
		getVerticalScrollBar().setUnitIncrement(5);
		this.setVerticalScrollBarPolicy(VERTICAL_SCROLLBAR_ALWAYS);

		this.cleanTable();
		MainCliente.requestRecentProducts();
		if (Constantes.updaterActive) {
			ts = new TimerScheduler(Constantes.updateRequest) {

				@Override
				public void toDo() {
					TableRecentProducts.this.cleanTable();
					MainCliente.requestRecentProducts();
		...
		p.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				MainApplication.pview.update(new Producto(ps.getIdProducto(), ps.getNameProducto(), ps.getPrecio(),
						ps.getInformacion(), ps.getTipo(), ps.getCategoria(), ps.isNegociable(), ps.getImgbytes(),
						ps.getStatus(), ps.getCreateDate()), ps.getUsuario());
	...
	public void createNewRow(String usuario, int id, String producto, String informacion, boolean negociable,
			int categoria, int tipo, double price, byte[] imageBytes, int views, int status, TimeCalendar date) {
		String palabra = "Si";
		if (!negociable) {
			palabra = "No";
		}
		...
		lTime.setText(TimeCalendar.parseo(date));
		...
		addNewCell(p, new SafeProducto(usuario, id, producto, informacion, negociable, categoria, tipo, price,
				imageIcon.getImage(), imageBytes, status, date));
